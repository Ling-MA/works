...
public class Swimmer extends S_scheduler {
	...
	public Swimmer(String name, BasketPool bp, CubiclePool cp, int t_undr, int t_swim, int t_dr) {
		...
		this.addAction(new TakeResourceAction(name, busr, bp));// take backet
																// ressource

		this.addAction(new TakeResourceAction(name, cusr, cp));// take cubicle
																// ressource

		this.addAction(new Foreseeable(name, "undressing", t_undr));// get
																	// undressed
